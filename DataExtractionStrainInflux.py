...
cwdDictionary = {}
#cwdDictionary['/home/cerecam/Desktop/RVE_25_34_42_50/2M_NEW_96x96x96_25PER/'] = ['S1_25PER_NoUEL2.odb']
...
#
# cwdDictionary['/home/cerecam/Desktop/RVE_25_34_42_50/2M_NEW_96x96x96_42PER/'] = ['S1_42PER_NoUEL.odb']
...
for cwd,odblist in cwdDictionary.items():
    ...
    with open(cwd + Z1_Poly_ele_file, 'r') as fname:
        Z1_P_ele = list(csv.reader(fname, delimiter=","))
    Z1_P_ele = Z1_P_ele[1:]
    ...
    for odbName in odblist:
        ...
        for frame in allFrames:
            ...
            Eav_P = np.sum(strainListP,0) / len(Z1_P_ele)
            ...
            print >> sys.__stdout__, 'frame ' + str(lastFrameValue + frame.frameValue) + ' complete'
            print >> sys.__stdout__, 'Average strain of poly elements: \t' + str(Eav_P[0:3])
        lastFrameValue = lastFrameValue + frame.frameValue

    with open(cwd + fOut + '.csv', 'w') as fname:
        csvWriter = csv.writer(fname, delimiter=',')
        csvWriter.writerow(['Frame time: '] + [x[0] for x in eav_poly])
