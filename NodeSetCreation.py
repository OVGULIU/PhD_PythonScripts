...
abaqus viewer noGui=/home/cerecam/Desktop/GIT/PhD_PythonScripts/NodeSetCreation.py
"""


def writefile(fname, node_values, nset_name, instance_name):
    # Creates file will heading '*Nset, nset='+str(nset)+', instance=RVE\n'
    # Writes first entry from list of lists given by values argument.
    fwrite = open(fname, 'w+')
    fwrite.write('*Nset, nset=' + str(nset_name) + ', instance=' + str(instance_name) + '\n')
    c = 1
    for nval in node_values:
        if c % 14 == 0:
            fwrite.write(str(nval[0]) + '\n')
            c += 1
        else:
            fwrite.write(str(nval[0]) + ', ')
            c += 1
...
def writefile2(fname, values_towrite, file_heading):
    # Creates file with heading 'heading'
    # Writes entries from list given by values argument.
    fwrite = open(fname, 'w')
    fwrite.write(file_heading)
    count = 1
    for val in values_towrite:
        if count % 14 == 0:
...
# filename2 = currentwd + '/nodeSets/SingleNodeSet.inp'
# F2 = open(filename2,'w+')
...
nodeSets = {'Right': Right, 'Left': Left, 'Top': Top, 'Bottom': Bottom, 'Back': Back, 'Front': Front,
            'TopFULL': TopFull, 'BottomFULL': BottomFull, 'BackFULL': BackFull, 'FrontFULL': FrontFull,
            'RightFULL': RightFull, 'LeftFULL': LeftFull,
            'R_T': R_T, 'R_Bot': R_Bot, 'R_F': R_F, 'R_B': R_B, 'R_T_F': R_T_F, 'R_T_B': R_T_B, 'R_Bot_F': R_Bot_F,
            'R_Bot_B': R_Bot_B,
            'L_T': L_T, 'L_Bot': L_Bot, 'L_F': L_F, 'L_B': L_B, 'L_T_F': L_T_F, 'L_T_B': L_T_B, 'L_Bot_F': L_Bot_F,
            'L_Bot_B': L_Bot_B,
            'T_F': T_F, 'T_B': T_B, 'Bot_F': Bot_F, 'Bot_B': Bot_B}
for key, value in nodeSets.items():
    filename = currentwd + '/nodeSets/' + str(key) + '.inp'
...
SetNames = {material1 + 'Nodes': Mat1NodeArray, material2 + 'Nodes': Mat2NodeArray, material2 + 'Node_Front': Mat2Front,
            ...
            material2 + '_Less_' + material1 + 'Nodes': Mat1LessMat2,
            material1 + '_Less_' + material2 + 'Nodes': Mat2LessMat1}

for key, vals in SetNames.items():
    filename = currentwd + '/nodeSets/' + key + '.inp'
...
f = open(currentwd + '/nodeSets/' + 'NodesetList.inp', 'w')
