...
cwd = '/home/cerecam/2M_128x128x128_full'
nodefile = 'Nodes.inp'
...
x0_ele, x1_ele = [min(poly_element)], [min(poly_element)]
y0_ele, y1_ele = [min(poly_element)], [min(poly_element)]
z0_ele, z1_ele = [min(poly_element)], [min(poly_element)]
...
for element in elelist:
    ...
    if all([x < centroid[i] for i, x in enumerate([xmin, ymin, zmin])]) and \
            all([x > centroid[i] for i, x in enumerate([xmax, ymax, zmax])]):
        count += 1
    else:
        if x_cent <= xmin:
            x0_ele.append(int(element[0]))
        if x_cent >= xmax:
            x1_ele.append(int(element[0]))
        if y_cent <= ymin:
            y0_ele.append(int(element[0]))
        if y_cent >= ymax:
            y1_ele.append(int(element[0]))
        if z_cent <= zmin:
            z0_ele.append(int(element[0]))
        if z_cent >= zmax:
            z1_ele.append(int(element[0]))
